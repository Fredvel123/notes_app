...
describe('POST Create user', () => {
    ...
    it('password incorrect - password length', async () => {
        ...
        expect(response.body.message).toBe('Your password must be greater than 5 and less than 25 characteres')
        expect(response.body.isCreated).toBe(false)
    });
    it('username incorrect - username length', async () => {
        ...
        expect(response.body.message).toBe('Username must be greater than 5 and less than 30 characteres')
        expect(response.body.isCreated).toBe(false)
    });
    it('user craeted successfully', async () => {
        const response = await request(app)
            .post(signUrl)
            .send(user)
        expect(response.body.message).toBe('Your account was created successfully')
    ...
    it('email already exists', async () => {
        const response = await request(app)
            .post(signUrl)
            .send(user)
        expect(response.body.message).toBe('The email is already used, please add a new email')
        expect(response.body.isCreated).toBe(false)
    });
});
describe('POST Create user', () => {
